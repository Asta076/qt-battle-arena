...
void PvpArenaWidget::spawnProjectile(const PvpFighterAnim& fighter, int owner)
{
    ...
    projectile.velocity = dir * m_logic.projectileSpeed();
...
void PvpArenaWidget::updateProjectiles()
{
    for (int i = m_projectiles.size() - 1; i >= 0; --i) {
        ...
        if (outOfBounds || projectile.lifeTicks > m_logic.projectileMaxTicks()) {
...
void PvpArenaWidget::applyKnockback(PvpFighterAnim& target, const QPointF& sourcePos)
{
    ...
    target.pos = clampToArena(target.pos + dir * m_logic.knockbackDistance());
}
                                 ...
                                 const QString& winnerText)
{
    ...
    target.hp -= damage;
    target.hitFlashTicks = m_logic.hitFlashTicks();
